...
def embed_to_circle(entities, d):
    order = tsp_solver_dp(entities, d)[1]
    result = []
    for i in order:
...
def optimize_placement(positions, d1, d2):
    ...
    while True:
        ...
        for i in range(k):
            for j in range(len(positions[i])):
                #print(loop_iteration, i, j)
                #distortion = total_distortion(positions, d1, d2)
                tmp = adjust(i,j,positions,d1,d2)
                #new_distortion = total_distortion(positions, d1, d2)
                #if not new_distortion <= distortion+eps:
                #    print(loop_iteration, i, j, tmp, new_distortion, distortion)
                #assert(new_distortion/distortion<=1.01)
...
def correct_positions(final_result, radius):
    ...
    for i in range(len(radius)):
        for j in range(len(final_result[i])):
            final_result[i][j] = (final_result[i][j][0], correct(final_result[i][j][1], radius[i]))
    return final_result
