...
def fit_transform(distance_matrix, min_dist=0, number_of_rings=4):
    ...
    for i in range(len(order)):
        ...
        if position_counter in partition[best_r] or (i!=0 and order[i-1]["group"] != e["group"]):
            position_counter+=1
    ...
    for p in range(position_counter):
        ...
        for i in range(len(partition)):
            if p in partition[i]:
                first_obj.append(partition[i][p])
            if p+1 in partition[i]:
                second_obj.append(partition[i][p+1]) 
